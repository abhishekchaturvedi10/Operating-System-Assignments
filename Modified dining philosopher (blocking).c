...
void *eat_food(void *arg) {
    ...
    while(1) {
    	...
    	// This puts a wait condition on the forks to be used by the current philospher so that the philospher can access these forks whenever they are free
    	wait(&forks[*i]);
    	wait(&forks[(*i+4)%5]);
       ...
       //Print the philospher number, its thread ID and the number of the forks it uses for eating 
    	...
    	// This signals the other philospher threads that the bowls are    available for eating 
    	signal(&bowls);
